...
#include <sys/cdefs.h>
__FBSDID("$FreeBSD: head/sys/dev/usb/wlan/if_run.c 301302 2016-06-04 07:18:39Z adrian $");
    ...
    struct usb_device_request *req, void *data)
{
	...
	if (err != 0) {
		device_printf(sc->sc_dev, "Control request failed, %s \n",
			usbd_errstr(err));
		device_printf(sc->sc_dev, "Control request error val: %d tries: %d\n",err, ntries);
	}
	return (err);
}
...
static int
run_write_4(struct run_softc *sc, uint16_t reg, uint16_t val1, uint16_t val2)
{
